...
public class Protivnik extends JFrame {
	...
	public Protivnik() {
		...
		JButton anotherTry = new JButton();
		anotherTry.setText(
				"                                                           Protivník trefil loï, nechat ho hrát                                                           ");
		anotherTry.setEnabled(false);
		Random r = new Random();
		JLabel instrukce = new JLabel();
		anotherTry.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					Game.Attack();
					if (Game.enemyPlaying == false) {
						anotherTry.setEnabled(false);
					}
		...
		instrukce.setText("Zde hádáte lodì protivníka");
		int CisloTlacitka = 0;
								...
								for (int i = 0; (btn - i) >= 0; i += 10) {
									if ((btn - i - 10) < 0) {
										...
										for (int x = 0; (btn + x) < 100; x += 10) {
											if ((btn + x + 10) >= 100) {
												if (ship[btn + x]
														&& !getContentPane().getComponent(btn + x).isEnabled()) {
													try {
